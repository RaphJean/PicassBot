...
from research.strategies import GreedySearch, RandomShootingMPC, GeneticSearch, CEMSearch, MCTSSearch, PolicyStrategy, LatentMPC
...
def main():
    ...
    base_output_dir = os.path.join("research_output", sub_dir)
    ...
    if args.strategy in ["latent_mpc", "all"]:
        if not args.joint_model_path:
            ...
            print("\nRunning Latent MPC...")
            latent_mpc = LatentMPC(world_config, target, step_penalty=args.step_penalty, joint_model_path=args.joint_model_path)
            history_latent_mpc = latent_mpc.run(max_steps=30, horizon=5, num_sequences=20)
